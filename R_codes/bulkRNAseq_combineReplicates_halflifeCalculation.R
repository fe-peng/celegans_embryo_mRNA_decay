...
## Import ERCC-normalized counts data for each replicate
repA_FC <- read.csv(file = 'repA_counts30_FC.csv')
repB_FC <- read.csv(file = 'repB_counts30_FC.csv')
repC_FC <- read.csv(file = 'repC_counts30_FC.csv')

## Merge to only keep genes shared in common with all 3 reps
repABC_FC <- merge(repA_FC, repB_FC, by = "gene")
repABC_FC <- merge(repABC_FC, repC_FC, by = "gene")

## Filter individual replicate FC dataframes to only keep genes kept between all 3
repA_FCshared <- repA_FC %>%
  filter(gene %in% repABC_FC$gene)

repB_FCshared <- repB_FC %>%
  filter(gene %in% repABC_FC$gene)

repC_FCshared <- repC_FC %>%
  filter(gene %in% repABC_FC$gene)
...
repA_FCshared_long <- gather(repA_FCshared, key = rep_timept, value = count, -gene) %>%
  separate(rep_timept, into = c("rep", "timept"))

repB_FCshared_long <- gather(repB_FCshared, key = rep_timept, value = count, -gene) %>%
  separate(rep_timept, into = c("rep", "timept"))

repC_FCshared_long <- gather(repC_FCshared, key = rep_timept, value = count, -gene) %>%
  separate(rep_timept, into = c("rep", "timept"))

## Combine dataframes vertically
repABC_FC_long <- rbind(repA_FCshared_long, repB_FCshared_long, repC_FCshared_long)

## Make timept column values numeric
repABC_FC_long$timept <- as.numeric(as.character(repABC_FC_long$timept))
...
param_RSS[,2:14] <- (param_RSS[,2:14])^2

## New column that is sum of residuals
param_RSS$RSS <- rowSums(param_RSS[ , 2:14])
...
param_TSS[,2] <- ((param_TSS[,2])-(param_TSS[,15]))^2
param_TSS[,3] <- ((param_TSS[,3])-(param_TSS[,15]))^2
param_TSS[,4] <- ((param_TSS[,4])-(param_TSS[,15]))^2
param_TSS[,5] <- ((param_TSS[,5])-(param_TSS[,15]))^2
param_TSS[,6] <- ((param_TSS[,6])-(param_TSS[,15]))^2
param_TSS[,7] <- ((param_TSS[,7])-(param_TSS[,15]))^2
param_TSS[,8] <- ((param_TSS[,8])-(param_TSS[,15]))^2
param_TSS[,9] <- ((param_TSS[,9])-(param_TSS[,15]))^2
param_TSS[,10] <- ((param_TSS[,10])-(param_TSS[,15]))^2
param_TSS[,11] <- ((param_TSS[,11])-(param_TSS[,15]))^2
param_TSS[,12] <- ((param_TSS[,12])-(param_TSS[,15]))^2
param_TSS[,13] <- ((param_TSS[,13])-(param_TSS[,15]))^2
param_TSS[,14] <- ((param_TSS[,14])-(param_TSS[,15]))^2
...
repA_nls_all <- read.csv(file = 'repA_nls_halflives_count30.csv')
repB_nls_all <- read.csv(file = 'repB_nls_halflives_count30.csv')
repC_nls_all <- read.csv(file = 'repC_nls_halflives_count30.csv')

## Only keep 'gene' and 'half_life' columns
repA_nls_all <- repA_nls_all[ , c("gene", "half_life")] 
repB_nls_all <- repB_nls_all[ , c("gene", "half_life")] 
repC_nls_all <- repC_nls_all[ , c("gene", "half_life")] 

## Rename columns so that half-lives reflect which rep they came from 
names(repA_nls_all)[names(repA_nls_all) == 'half_life'] <- 'half_life_A'
names(repB_nls_all)[names(repB_nls_all) == 'half_life'] <- 'half_life_B'
names(repC_nls_all)[names(repC_nls_all) == 'half_life'] <- 'half_life_D'

## Merge all dataframes by gene to only keep genes shared across all reps
repAB_CV <- merge(repA_nls_all, repB_nls_all, by = "gene")
repABC_CV <- merge(repAB_CV, repC_nls_all, by = "gene")

## Calculate standard deviation and mean for each gene across replicates
repABC_CV <- transform(repABC_CV, stdev = apply(repABC_CV[, 2:4], 1, sd))
repABC_CV <- transform(repABC_CV, mean = apply(repABC_CV[, 2:4], 1, mean))

## Divide stdev by mean half-life to get coefficient of variation
repABC_CV <- transform(repABC_CV, CV = stdev / mean)
...
CI_CV <- merge(param_all_CI, repABC_CV, by = "gene")
...
write.table(CIFC3_CV50_CIFC4andCV75hl100, file = "bulk_halflives_CIFC3_CV50_CIFC4andCV75hl100_count30.txt", sep = "\t", row.names = TRUE, quote = FALSE)
